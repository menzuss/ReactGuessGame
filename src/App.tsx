...
function App() {
  ...
  function restartGame() {
    const confirm = window.confirm("Você deseja realmente reiniciar?")
    if (confirm) {
      startGame()
    }
  }
  ...
  function handleConfirm() {
    ...
    if (exists) {
      return alert("Ja utilizou esta letra!")
    }
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (lettersUsed.length === attemptLimit) {
        return endGame("Que pena, vocêusou todas as tentativas!")
      }
    }, 200)
  ...
  return (
    <div className={styles.container}>
      <main>
        <Header current={lettersUsed.length} max={challenge.word.length + ATTEMPTS_MARGIN} onRestart={restartGame} />
